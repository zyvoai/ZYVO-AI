...
function globalConfigFile() {
  const candidates = ["zyvo.jsonc", "zyvo.json", "config.json"].map((file) =>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "zyvo.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "zyvo.jsonc"), env))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        for (const dir of directories) {
          if (dir.endsWith(".zyvo") || dir === Flag.OPENCODE_CONFIG_DIR) {
            for (const file of ["zyvo.json", "zyvo.jsonc"]) {
